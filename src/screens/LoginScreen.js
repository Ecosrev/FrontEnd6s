    ...
    const handleLogin = async (values) => {
        ...
        try {
            ...
            if (!access_token) {
                ...
                showAlert('Erro', 'Token de acesso não recebido.');
                return;
            }
            ...
            if (!loginSuccess) {
                ...
                showAlert('Erro', 'Erro ao salvar dados de autenticação.');
                return;
            }
            ...
            try {
                ...
                if (userInfoResponse.data.resetPasswordToken) {
                    showAlert(
                        'Senha Temporária',
                        'Login com senha temporária. Você precisa criar uma nova senha.',
                        async () => {
            ...
            if (error.response?.data?.errors) {
                ...
                showAlert('Erro no login', mensagens);
            } else if (error.response?.status === 401) {
                setErrorMessage('Email ou senha incorretos.');
                showAlert('Erro de autenticação', 'Email ou senha incorretos.');
            } else {
                setErrorMessage('Erro de conexão com o servidor.');
                showAlert('Erro de conexão', 'Não foi possível conectar ao servidor. Verifique sua internet.');
            }
        } finally {
            setIsLoading(false);
        }
